...
"""
                ...
                Import de .VCF et exports VCF/TXT/ODT/ODS/PDF
...
def split_phones(raw):
    """Retourne une liste de numéros depuis un champ libre (séparateurs: \n ; / , |)."""
    if not raw:
        return []
    parts = re.split(r"[\n;/,|]+", str(raw))
    out = []
...
def french_date_str(dt):
    """Retourne une date en français (ex: 02 janvier 2026) sans dépendre de la locale système."""
    if dt is None:
        dt = datetime.now()
    return f"{dt.day:02d} {FRENCH_MONTHS.get(dt.month, str(dt.month))} {dt.year}"

def split_address_lines(raw, max_lines=2):
    """Découpe une adresse libre (VCF/PDF/TXT) en lignes propres."""
    if not raw:
        return []
    s = str(raw).replace('\\n', '\n').replace('\r', '\n')
...
def vcf_escape_text(s):
    """Échappe un texte pour vCard (VERSION:3.0)."""
    if s is None:
        return ''
    s = str(s)
...
def parse_vcf(path):
    ...
    for card in cards:
        ...
        # Adresse (champ libre) : LABEL en priorité, sinon ADR
        addr = ''
        label_match = re.search(r'(?mi)^\s*(?:item\d+\.)?LABEL(?:;[^:]*)?:(.+)$', card)
        if label_match:
            label_val = label_match.group(1).strip()
            header = label_match.group(0)
            if re.search(r'ENCODING=QUOTED-PRINTABLE', header, flags=re.I):
                label_val = decode_quoted_printable(label_val)
            addr = label_val.replace('\\n', '\n').strip()
        else:
            adr_match = re.search(r'(?mi)^\s*(?:item\d+\.)?ADR(?:;[^:]*)?:(.+)$', card)
            if adr_match:
                adr_val = adr_match.group(1).strip()
                header = adr_match.group(0)
                if re.search(r'ENCODING=QUOTED-PRINTABLE', header, flags=re.I):
                    adr_val = decode_quoted_printable(adr_val)
                parts = adr_val.split(';')
                ...
                lines_addr = [street]
                line2 = ' '.join([postal, city]).strip()
                if line2:
                    lines_addr.append(line2)
                line3 = ' '.join([region, country]).strip()
                if line3:
                    lines_addr.append(line3)
                addr = '\n'.join([x for x in lines_addr if x]).strip()
        tels = re.findall(r'(?mi)^\s*(?:item\d+\.)?TEL[^:]*:([^\r\n]+)', card)
        if not tels:
            continue
        phones = [format_phone(t) for t in tels]
        ...
        if not name:
            name = number
        contacts.append({'name': name, 'number': number, 'address': addr, 'enabled': None, 'widgets': {}})
    return contacts
...
def make_logical_half_pages(contacts_enabled, contact_pt, heading_pt, page_h_pts, top_margin_pts, bottom_margin_pts):
    ...
    addr_line_height = int(max(contact_pt - 1, 7) * 1.05)
    ...
    for letter in sorted(grouped.keys()):
        ...
        for ct in grouped[letter]:
            ...
            if used + line_height > usable:
                push()
            phones = split_phones(ct.get('number',''))
            if not phones:
                continue
            curr.append(('L', f"{ct['name']}|||{phones[0]}"))
            used += line_height
            for extra in phones[1:]:
                if used + line_height > usable:
                    push()
                curr.append(('L', f"|||{extra}"))
                used += line_height
            addr = (ct.get('address') or '').strip()
            if addr:
                wrapped = textwrap.wrap(addr.replace('\\n', ' '), width=60)
                wrapped = wrapped[:2]
                # Ligne vide au-dessus de l'adresse (pour les exports PDF)
                if used + line_height > usable:
                    push()
                curr.append(('E',))
                used += line_height
                for wline in wrapped:
                    if used + addr_line_height > usable:
                        push()
                    curr.append(('A', wline))
                    used += addr_line_height
                # Ligne vide en dessous de l'adresse (pour les exports PDF)
                if used + line_height > usable:
                    push()
                curr.append(('E',))
                used += line_height
...
class KLMEditor(tk.Tk):
    ...
    def create_interface(self):
        ...
        ttk.Button(btn_frame, text=BUTTON_LABELS['export_vcf'], command=self.export_vcf, style=TTK_STYLE_NAME).pack(side='left', padx=4)
    ...
    def load_vcf(self):
        ...
        if not new:
            ...
            return
        existing_keys = set((c['name'].strip().lower(), (split_phones(c.get('number',''))[0] if split_phones(c.get('number','')) else '').strip()) for c in self.contacts)
        added = 0
        for ct in new:
            key = (ct['name'].strip().lower(), (split_phones(ct.get('number',''))[0] if split_phones(ct.get('number','')) else '').strip())
    ...
    def update_preview(self):
        ...
        for letter in sorted(grouped.keys()):
            ...
            for c in sorted(grouped[letter], key=lambda x: x['name'].lower()):
                ...
                # --- Téléphones (alignés sur la colonne numéro) ---

                phones = split_phones(c.get('number', ''))

                first = phones[0] if phones else ''

                line = f"{display_name}{' ' * pad}{first}\n"

                self.preview_text.insert('end', line, 'contact')

                # Lignes supplémentaires : même colonne "numéro", sans label

                phone_col = max_name + MIN_SPACES

                for extra in phones[1:]:

                    self.preview_text.insert('end', f"{' ' * phone_col}{extra}\n", 'contact')

                # --- Adresse (champ libre) : 2 lignes max, même colonne "numéro" ---

                addr = (c.get('address') or '').strip()

                if addr:

                    addr_lines = textwrap.wrap(addr.replace('\n', ' '), width=44)[:2]

                    for al in addr_lines:

                        self.preview_text.insert('end', f"{' ' * phone_col}{al}\n", 'addr')
            self.preview_text.insert('end', '\n')
        ...
        try:
            ...
            self.preview_text.tag_configure('addr', font=('Helvetica', max(csize-1, 9), 'italic'))
        except Exception:
            pass
    ...
    def export_vcf(self):
        if not self.contacts:
            messagebox.showinfo('VCF Export', 'Aucun contact.')
            return
        path = filedialog.asksaveasfilename(defaultextension='.vcf', filetypes=[('vCard', '*.vcf')])
        if not path:
            return
        exported = 0
        with open(path, 'w', encoding='utf-8') as f:
            for c in sorted(self.contacts, key=lambda x: x['name'].lower()):
                if not (c.get('enabled') and c['enabled'].get()):
                    continue
                ...
                f.write('BEGIN:VCARD\n')
                f.write('VERSION:3.0\n')
                f.write(f'FN:{vcf_escape_text(name)}\n')
                f.write(f'N:{vcf_escape_text(name)};;;;\n')
                for p in phones:
                    f.write(f'TEL:{vcf_escape_text(p)}\n')
                if addr:
                    f.write(f'LABEL:{vcf_escape_text(addr)}\n')
                f.write('END:VCARD\n')
                exported += 1
        messagebox.showinfo('VCF Export', f'{exported} contact(s) exporté(s).')
    def export_txt(self):
        if not self.contacts:
        ...
        with open(path, 'w', encoding='utf-8') as f:
            for letter in sorted(grouped.keys()):
                ...
                for c in grouped[letter]:
                    ...
                    if not phones:
                        continue
                    line = f"{display_name}{' ' * pad}{phones[0]}\n"
                    f.write(line)
                    for extra in phones[1:]:
                        f.write(f"{' ' * (max_name + MIN_SPACES)}{extra}\n")
                    addr_lines = split_address_lines(c.get('address',''), max_lines=2)
                    if addr_lines:
                        # Ligne vide au-dessus et en dessous de l'adresse (lisibilité)
                        f.write("\n")
                        for a in addr_lines:
                            f.write(f"{' ' * (max_name + MIN_SPACES)}{a}\n")
                        f.write("\n")
    ...
    def export_odt(self):
        if not ODF_AVAILABLE:
            ...
            return
        if not self.contacts:
            messagebox.showinfo('ODT Export', 'Aucun contact.')
            return
        ...
        # Tableau 3 colonnes : Nom / Téléphone(s) / Adresse
        table = Table(name="Contacts")
        doc.text.addElement(table)

        # Déclarer explicitement les colonnes (LibreOffice est parfois tatillon)
        for _ in range(3):
            table.addElement(TableColumn())

        header = TableRow()
        for title in ("Nom", "Numéro(s)", "Adresse"):
            cell = TableCell()
        ...
        for letter in sorted(grouped.keys()):
            sep = TableRow()
            cell = TableCell(numbercolumnsspanned="3")
            cell.addElement(P(text=f"— {letter} —"))
            sep.addElement(cell)
            # Couvrir les cellules incluses dans le spanning (sinon colonnes décalées)
            sep.addElement(CoveredTableCell())
            sep.addElement(CoveredTableCell())
            table.addElement(sep)
            ...
            for c in grouped[letter]:
                ...
                # Nom
                cell_name = TableCell()
                cell_name.addElement(P(text=c.get('name','')))
                row.addElement(cell_name)

                # Téléphones (un par ligne)
                cell_tel = TableCell()
                for p in split_phones(c.get('number','')):
                    cell_tel.addElement(P(text=p))
                row.addElement(cell_tel)

                # Adresse (2 lignes max)
                cell_addr = TableCell()
                for a in split_address_lines(c.get('address',''), max_lines=2):
                    cell_addr.addElement(P(text=a))
                row.addElement(cell_addr)
    ...
    def export_ods(self):
        if not ODF_AVAILABLE:
            ...
            return
        if not self.contacts:
            messagebox.showinfo('ODS Export', 'Aucun contact.')
            return
        ...
        try:
            ...
            # Déclarer explicitement les colonnes
            for _ in range(3):
                table.addElement(TableColumn())
...
class LivretWindow(tk.Toplevel):
    def __init__(self, master, contacts, logo_path=None):
        ...
        self.date_var = tk.StringVar(value=PDF_DEFAULTS['date_text'].format(french_date_str(datetime.now())))
    ...
    def _generate_fold_2(self, c, pw, ph, contacts_enabled): # Pliage 2 plis
        ...
        addr_pt = max(pdf_contact_pt - 1, 7)
        addr_line_height = int(addr_pt * 1.07)
        ...
        def render_half(x_offset, half_index):
            ...
            y = ph - top_margin
            line_height = approx_line_height
            if half_index == 0:
                return
            ...
            for item in lines:
                if item[0] == 'B':
                    ...
                    _, atext = item
                    c.setFont('Helvetica-Oblique', addr_pt)
                    c.drawString(inner_left + int(inner_width * 0.03), y, atext)
                    y -= addr_line_height
        ...
        def draw_text_in_zone(cobj, x, y, w, h, page_num):
            ...
            for item in lines:
                if item[0] == 'B':
                    ...
                    _, atext = item
                    cobj.setFont('Helvetica-Oblique', max(pdf_contact_pt - 1, 7))
                    cobj.drawString(inner_left + int(inner_width * 0.03), cur_y, atext)
                    cur_y -= int(pdf_contact_pt * 1.07)
